import { isValidKeymapShape, type Keymap } from '../src/data/types'
...
export async function fetchSharedKeymap(id: string, timeoutMs = 3000): Promise<SharedKeymapRow | null> {
  ...
  try {
    const res = await fetch(
      `${base}/rest/v1/shared_keymaps?id=eq.${id}&select=id,name,author,description,keymap`,
      { headers: { apikey: key, Authorization: `Bearer ${key}` }, signal: ctrl.signal },
    )
    if (!res.ok) return null
    const rows: unknown = await res.json()
    const row = Array.isArray(rows) ? rows[0] : null
    if (!row || !isValidKeymapShape(row.keymap)) return null
    return {
      id: row.id,
